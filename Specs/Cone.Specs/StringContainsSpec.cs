...
namespace Cone
{
    ...
    public class StringContainsSpec
    {
        public void check_success() {
            ...
            Check.That(() => expect.Check().Equals(new CheckResult(true, Maybe<object>.Some("Hello"), Maybe<object>.None)));
        }

        public void check_fail() {
            Expression<Func<bool>> body = () => "123".Contains("ABC");
            IExpect expect = ContainsExpect(body, "123", "ABC");
            Check.That(() => expect.Check().Equals(new CheckResult(false, Maybe<object>.Some("123"), Maybe<object>.None)));
        }
        ...
        public void property_example() {
            var foo = this;
            Check.That(() => foo.HelloWorld.Contains("World"));
        }
